...
- **`novel-isr dev/start` 无条件覆盖 `ssr.config.ts` 的监听端口**：Commander 此前把 `3000`
  作为 `--port` 的默认参数，即使调用方没有传 CLI flag，也会在加载配置后覆盖
  `server.port`（包括业务通过 `PORT` 环境变量解析出的端口）。现在 dev/start 仅在显式传入
  `--port` / `--host` 时覆盖配置；新增命令参数回归测试，覆盖无参数与显式覆盖两种路径。
