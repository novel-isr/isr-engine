...
| `/sitemap.xml` 500 | 站点 base URL 未设 | dev 自动兜底；prod 在 `ssr.config.ts runtime.site` 配置 |
...
| dev inspector 里 i18n 是 `local-fallback` | 配置 API 未启动、`runtime.services.api` 不对，或远端返回非 2xx | 启动配置 API，确认 `ssr.config.ts runtime.services.api` 和 `runtime.i18n.endpoint` 指向正确 |
