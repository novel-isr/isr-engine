...
| `X-Cache-Stale-Reason` | STALE 时的原因：`swr-fresh` / `swr-bg-pending` / `swr-bg-failed-recent` |
| `X-Cache-Age` | 缓存条目年龄（秒；HIT/STALE 时） |
| `X-Cache-Key` | 缓存键（便于排错） |

**Stale Reason 三态：** `swr-fresh` 刚进 SWR 窗口属正常，`swr-bg-pending` 后台重渲在跑也正常，`swr-bg-failed-recent` 表示过去 60s 内 bg 重渲失败过（5xx / 超时）—— 说明上游可能挂了，用户在持续看旧数据，立即排查。

排错时还可以查 [`/__isr/cache/inventory` admin 端点](./observability.md#__isrcacheinventory-admin-端点)，回答「现在哪些条目过期了但没人请求」这种 Prometheus 看不见的问题。

### 缓存键的隔离维度

基础 key 之上有两个可选隔离后缀，按需启用：

| 后缀 | 开关 | 何时用 |
|---|---|---|
| `\|h=<host>` | `runtime.hostIsolation: true` | **多域名打到同一 SSR 进程且按域名出不同内容**（白标站 / 渠道站 / tenant-by-domain）。不开会跨站串缓存。单站点（一进程一域名，业界默认形态）保持关闭，避免 www/裸域/内网探活多个 Host 别名造成碎片化 |
| `\|v=<digest>` | 配置 `runtime.experiments` 后自动启用 | A/B 实验影响 HTML 结构时，同路径不同变体各自缓存，互不串货 |

`revalidatePath` 不区分隔离后缀 —— 清除该路径**所有** host / variant 的条目。
...
- 每个失败都会单独 log + 递增 `isr_invalidator_failures_total{kind,target}` Prometheus（`target` 是归一化路径或 tag 名，能定位到具体业务对象）
  counter，方便告警
