...
- `redis`：分布式 ISR 缓存和跨实例失效广播。`runtime.redis.url/host` 或 `REDIS_URL/REDIS_HOST` 非空时 engine 自动启用 Redis；没有 Redis 时自动使用进程内 memory cache。页面 TTL 不在这里配置，而在 `routes[*].ttl` / `isr.revalidate`。
- `sentry`：服务端错误监控。
...
- `store`：默认 `auto`；检测到 `runtime.redis` / `REDIS_URL` / `REDIS_HOST` 才用 Redis，否则进程内 memory。
