...
- `sentry`：服务端错误监控。
- `rateLimit`：站点入口限流；默认是进程内 memory fixed-window counter，重启清空，不跨 pod 共享。需要分布式限流时显式配置 `store: 'redis'` 或 `store: 'auto'`。
...
runtime: {
  redis: process.env.REDIS_URL
    ? { url: process.env.REDIS_URL, keyPrefix: 'novel:' }
    : undefined,
  rateLimit: {
    store: 'memory',
    windowMs: 60_000,
...
- `store`：`memory` 是默认值，进程内 LRU；`redis` 使用 `runtime.redis` / `REDIS_URL` / `REDIS_HOST`；`auto` 检测到 Redis 配置才用 Redis。
...
如果同时配置 Sentry / Datadog / OTel exporter，第三方 vendor 会作为同一条
`runtime.telemetry.exporters` fan-out 出口执行；vendor 失败不会阻断 endpoint 上报或业务 `onError`。
