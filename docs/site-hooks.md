...
- `redis`：分布式 ISR 缓存和跨实例失效广播。`runtime.redis.url/host` 或 `REDIS_URL/REDIS_HOST` 非空时 engine 自动启用 Redis；没有 Redis 时自动使用进程内 memory cache。页面 TTL 不在这里配置，而在 `routes[*].ttl` / `revalidate`。
- `sentry`：服务端错误监控。
