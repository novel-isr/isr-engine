...
- `services.api`：默认后端 API base URL，例如书籍、用户、评分、配置中心、i18n、SEO 和 mock 数据。
...
## Rate limiting：不在 engine 范畴

业界共识（Next.js / Remix / Astro / SvelteKit / Nuxt 全部不做）：渲染框架不承担
rate limiting。生产场景去 CDN/WAF/Gateway（Cloudflare WAF / AWS WAF / Vercel Edge /
Kong / Envoy）；应用层兜底用独立 middleware（`express-rate-limit`），不进框架。
