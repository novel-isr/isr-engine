...
export class RSCRenderer {
  ...
  async renderRSCTree(
    ...
    try {
      ...
      this.logger.debug(
        `📊 组件分析完成: ${serverComponents.length} 个服务端组件, ${clientComponents.length} 个客户端组件`
      );

      // 第二步：渲染服务端组件
      const serverRenderResults = await this.renderServerComponents(serverComponents, context);
      ...
      const hybridTree = this.createHybridTree(componentTree, serverRenderResults, componentMap);
   ...
   * 分析组件树，分离服务端和客户端组件
   */
  private async analyzeComponentTree(element: React.ReactElement): Promise<{
  ...
  async initialize(): Promise<void> {
    this.logger.info('🚀 RSC 渲染器已初始化');
  }

  /**
   * 渲染 RSC（兼容性方法，代理到 renderRSCTree）
   */
  async renderRSC(element: React.ReactElement, context: RenderContext): Promise<{
    html: string;
    rscPayload: RSCPayload;
    clientBundle: string[];
  }> {
    return this.renderRSCTree(element, context);
  }
