...
export class RSCModule {
  ...
  async renderWithRSC(
    ...
    try {
      ...
      const resolvedElement = await this.resolveAsyncComponents(
        appElement,
        context,
        prefetchedData
      );
   ...
   * 预取 RSC 组件数据 - 生产级数据预取
   */
  private async prefetchRSCData(element: any, context: RenderContext): Promise<Map<string, any>> {
    ...
    if (typeof component === 'function') {
      try {
        ...
        if (comp._requiresServerData) {
          ...
          // 获取预取的数据 - 必须存在，不允许 fallback
          const componentData = prefetchedData.get(componentName);
          
          if (componentData === undefined) {
            throw new Error(`组件 ${componentName} 的数据预取失败，没有可用数据`);
          }
          ...
          const enhancedProps = {
            ...
            ...(componentName === 'SensitiveDataDemo' && {
              sensitiveData: this.processSensitiveData(componentData),
              internalAnalysis: this.processInternalAnalysis(componentData),
              personalDiscount: this.calculatePremiumDiscount(componentData),
            }),
          };
          ...
          // 递归处理子元素
          return await this.resolveAsyncComponents(
            enhancedElement,
            context,
            prefetchedData,
            depth + 1
    ...
    if (elementProps?.children) {
      ...
      if (Array.isArray(children)) {
        resolvedChildren = await Promise.all(
          children.map(child =>
            this.resolveAsyncComponents(child, context, prefetchedData, depth + 1)
          )
        );
      } else {
        resolvedChildren = await this.resolveAsyncComponents(
          children,
          context,
          prefetchedData,
          depth + 1
  ...
  private processSensitiveData(rawData: any): any {
    ...
    return {
      ...
      financial: {
        ...
        totalSpent:
          user.sensitiveInfo?.paymentHistory?.reduce(
      ...
      analytics: {
        ...
        averageCompletion:
          user.sensitiveInfo?.readingHistory?.length > 0
            ? user.sensitiveInfo.readingHistory.reduce(
  ...
  private processInternalAnalysis(rawData: any): any {
    const user = rawData?.user;
    if (!user) {
      throw new Error('内部分析数据缺少必要的用户信息');
    }
  ...
  private calculatePremiumDiscount(rawData: any): number {
    ...
    if (!config || !user) {
      throw new Error('个性化折扣计算缺少必要的配置或用户数据');
    }
  ...
  private isServerComponent(component: any): boolean {
    ...
    return componentString.includes("'use server'") || componentString.includes('"use server"');
  }
