...
[![Vite 8](https://img.shields.io/badge/Vite-8-646CFF.svg)](https://vitejs.dev/) [![React 19](https://img.shields.io/badge/React-19-61DAFB.svg)](https://react.dev/) [![Express 5](https://img.shields.io/badge/Express-5-000000.svg)](https://expressjs.com/) [![Node 22.21.1](https://img.shields.io/badge/Node-22.21.1-339933.svg)](https://nodejs.org/) [![Tests 728](https://img.shields.io/badge/Tests-728%20passing-brightgreen.svg)](./CHANGELOG.md)

> **v2.5.10（2026-05-29）** —— browser telemetry 上传退避保护：修复 endpoint 临时断连时
> flush interval 与 retry timer 叠加导致同批 payload 重复并发上传的问题；
> 同一 endpoint 同时只允许一个普通 flush，失败期间只由退避 retry 驱动。
> 详见 [CHANGELOG.md](./CHANGELOG.md#2510---2026-05-29)。
...
baseline：[`bench/baseline.json`](./bench/baseline.json) —— 由 [`bench/app/`](./bench/app/)
...
复现：`cd bench/app && pnpm run build && pnpm start`，
...
| 单元测试覆盖 | 数千用例 | ⚠️ | ✅ | 60 文件 / 728 tests / ~61% |
...
v2.3.1 收口了消费侧首跑 0 配置 + express 5 + 入口架构清理，
v2.5 系列补上 ISR 缓存可观测性（`X-Cache-Stale-Reason` / inventory 端点）、500 兜底页
和 telemetry 上传退避。但**离 1.0 stable 还差**：

✅ **稳的部分**：
- Flight 协议委托给官方 `@vitejs/plugin-rsc@^0.5.26`，不自维护
- 依赖全是工业级（Express 5 / Helmet / Prometheus / sitemap / lru-cache / ioredis）
- 728 tests / ~61% 行覆盖；CI 任何分支 push 都跑 lint+typecheck+test
...
- 浏览器侧没有持续跑的 e2e（728 测试主要是 Node 侧单测），靠人工
  smoke test。任何 React 19 / plugin-rsc / Vite 升级都需要先在 bench app 里手验
...
{ "dependencies": { "@novel-isr/engine": "^2.5.0" } }
...
| **`bench.yml`** | nightly 02:00 UTC + 手动 + perf-sensitive 路径 PR | 起 bench app + 跑 bench + 对比 `bench/baseline.json` | 信息性 fail（不阻断 release，只用于追踪退化） |
